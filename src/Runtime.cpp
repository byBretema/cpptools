...
WAI_NOINLINE WAI_FUNCSPEC int
             WAI_PREFIX(getExecutablePath)(char* out, int capacity,
                                  int* dirname_length) {
...
std::string Runtime::exePath() {
  std::string outPath{ "" };

  int _dirLength;
  int _absLength = wai_getExecutablePath(NULL, 0, &_dirLength);

  if (_absLength > 0) {
    char* path = (char*)malloc(_absLength + 1);
    if (!path) return "";

    wai_getExecutablePath(path, _absLength, &_dirLength);
    path[_dirLength] = '\0';
